...
load('/Users/plb/ML_CNBC/pmtk3-master/data/mnist1NNresults/mnist1NNresults.mat');
if 1
...
ypred = zeros(ntest,1);
if 1, wbar = waitbar(0,sprintf('%d of %d classified',0,ntest)); end % waitbar only works in Matlab
% Classify
for i=1:nbatches
    t = toc; if 1, waitbar(i/nbatches,wbar,sprintf('%d of %d Classified\nElapsed Time: %.2f seconds',(i-1)*(ntest/nbatches),ntest,t));end
    dst = sqDistance(Xtest(batches{i},:),Xtrain,XtestSOS(batches{i},:),XtrainSOS);
    [junk,closest] = min(dst,[],2);
    ypred(batches{i}) = ytrain(closest);
end
[result, dists] = flann_search(Xtrain,Xtest,5,params);
...
% perform the neares't-neighbor search
